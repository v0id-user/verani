# Verani Documentation

Complete documentation for Verani, a realtime framework for Cloudflare Workers.

## Getting Started

- [Installation](./getting-started/installation.md) - Prerequisites and installation
- [Quick Start Guide](./getting-started/quick-start.md) - Build your first Verani application
- [Troubleshooting](./getting-started/troubleshooting.md) - Common issues and solutions

## API Reference

- [Server API](./api/server.md) - Server-side API documentation
- [Client API](./api/client.md) - Client-side API documentation
- [Types](./api/types.md) - Type definitions
- [Utilities](./api/utilities.md) - Utility functions

## Guides

- [Configuration](./guides/configuration.md) - Wrangler configuration and setup
- [Deployment](./guides/deployment.md) - Deploying to Cloudflare
- [Monitoring](./guides/monitoring.md) - Logs and metrics
- [Scaling](./guides/scaling.md) - Performance and scaling strategies
- [RPC](./guides/rpc.md) - Remote Procedure Calls

## Examples

- [Basic Chat](./examples/basic-chat.md) - Simple chat room
- [Channels](./examples/channels.md) - Custom paths and multiple channels
- [Presence](./examples/presence.md) - User presence tracking
- [Authentication](./examples/authentication.md) - JWT authentication
- [Rate Limiting](./examples/rate-limiting.md) - Prevent spam
- [RPC](./examples/rpc.md) - Remote Procedure Calls

See [Examples Index](./examples/README.md) for all examples.

## Concepts

- [Architecture](./concepts/architecture.md) - System architecture and design
- [Actors and Channels](./concepts/actors-channels.md) - Core concepts
- [Hibernation](./concepts/hibernation.md) - Hibernation behavior
- [State Management](./concepts/state-management.md) - State types
- [RPC](./concepts/rpc.md) - Remote Procedure Calls
- [Isolation](./concepts/isolation.md) - Isolation strategies
- [Lifecycle](./concepts/lifecycle.md) - Connection lifecycle

## Security

- [Authentication](./security/authentication.md) - Verifying user identity
- [Authorization](./security/authorization.md) - What users can do
- [Input Validation](./security/input-validation.md) - Validating user input
- [Rate Limiting](./security/rate-limiting.md) - Preventing abuse
- [Vulnerabilities](./security/vulnerabilities.md) - Common security issues
- [Security Checklist](./security/checklist.md) - Production checklist

## Support

- GitHub Issues: [Report bugs](https://github.com/v0id-user/verani/issues)
- Discussions: [Ask questions](https://github.com/v0id-user/verani/discussions)
- Discord: [Join community](#) (coming soon)

